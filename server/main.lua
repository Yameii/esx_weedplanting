    ...
    ESX.RegisterUsableItem(item_name, function(source)
        ...
        if cops >= options[item_name].cops then
            ...
            TriggerClientEvent("pNotify:SendNotification", _source, {
                text = 'To start this operation must be at least '.. options[item_name].cops .. ' policemen on duty.',
                type = "error",
                timeout = 5000,
                layout = "centerLeft"
            })
            TriggerClientEvent("pNotify:SendNotification", _source, {
                text = 'Current number of police officers on duty: '.. cops ..'/'.. options[item_name].cops,
                type = "info",
                timeout = 6000,
                layout = "centerLeft"
            })
...
AddEventHandler("esx_receptury:statusSuccess", function(message, min, max, item)
    ...
    if(itemProps.limit < itemProps.count + amount) then
        ...
        TriggerClientEvent("pNotify:SendNotification", source, {
            text = 'You leave a part of the product because you do not have space in your pockets anymore.',
            type = "error",
            timeout = 2000,
            layout = "centerLeft"
        })
